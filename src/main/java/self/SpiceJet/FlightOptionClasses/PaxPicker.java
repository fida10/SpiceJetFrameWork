...
public class PaxPicker {
	private static void openPaxBox(){
		Actions a = new Actions(dr);
		a
				.moveToElement(dr.findElement(By.xpath(pro.getProperty("xpathOneWayDepPaxPicker")))) //the xpath to the passengers box
				.click()
				.build()
				.perform();
	}
	private static List<Integer> StringToIntForSelectBoxes(){ //converts provided string values in properties file to integers.
		String[] noOfEachPaxType = {pro.getProperty("OneWayNoOfAdults"), pro.getProperty("OneWayNoOfChildren"), pro.getProperty("OneWayNoOfInfants")};
	...
	public static void adultChildInfantNoPicker() throws InterruptedException{
		Thread.sleep(200);
		openPaxBox();
		Thread.sleep(200);
		//Adult
		Select adult = new Select(dr.findElement(By.cssSelector(pro.getProperty("cssSelectOneWayDepPaxPickerAdult"))));
		...
		//Child
		Select child = new Select(dr.findElement(By.cssSelector(pro.getProperty("cssSelectOneWayDepPaxPickerChild"))));
		...
		Select infant = new Select(dr.findElement(By.cssSelector(pro.getProperty("cssSelectOneWayDepPaxPickerInfant"))));
